...
const PrivateUserOutletRoute = () => {
	const { background } = useSelector((state) => state.common);
	const { user, privateProfile: userPrivateProfile, isPrivateProfileError } = useSelector((state) => state.user);
	const dispatch = useDispatch();
	const navigate = useNavigate();

	useEffect(() => {
		if (!user || isPrivateProfileError) navigate("/user/signin", { replace: true });
	...
	return user ? 
		<UserDashboardLayout privateProfile={userPrivateProfile} background={background}>
