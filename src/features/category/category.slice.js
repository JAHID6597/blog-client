...
export const getCategory = createAsyncThunk('category/get-category', async (slug, thunkAPI) => {
	try {
...
export const getCategories = createAsyncThunk('category/get-categories', async (data={}, thunkAPI) => {
	try {
		const { page, limit, search } = data;
        return await categoryService.getCategories(page, limit, search || "");
    }
    catch (error) {
        const message = (error.response && error.response.data) || (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
