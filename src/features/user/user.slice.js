...
export const signUp = createAsyncThunk("user/signup", async (formData, thunkAPI) => {
		try {
...
export const signIn = createAsyncThunk("user/signin", async (formData, thunkAPI) => {
    try {
...
export const logout = createAsyncThunk('user/logout', async (args, thunkAPI) => {
	try {
...
export const getPublicProfile = createAsyncThunk('user/get-public-profile', async (userName, thunkAPI) => {
	try {
...
export const getPrivateProfile = createAsyncThunk('user/get-private-profile', async (args, thunkAPI) => {
	try {
		const token = thunkAPI.getState().user.user.accessToken;
...
export const updateProfile = createAsyncThunk('user/update-profile', async (formData, thunkAPI) => {
	try {
		const token = thunkAPI.getState().user.user.accessToken;
...
export const deleteProfile = createAsyncThunk('user/delete-profile', async (args, thunkAPI) => {
	try {
		const token = thunkAPI.getState().user.user.accessToken;
...
export const changePassword = createAsyncThunk('user/change-password', async (formData, thunkAPI) => {
	try {
		const token = thunkAPI.getState().user.user.accessToken;
...
export const followUser = createAsyncThunk('user/follow-user', async (userName, thunkAPI) => {
	try {
		const token = thunkAPI.getState().user.user.accessToken;
...
export const getFollowerUsers = createAsyncThunk('user/follower-user', async (data={}, thunkAPI) => {
	try {
		const { userName, page, limit, search } = data;
        return await userService.getFollowerUsers(userName, page, limit, search || "");
    }
    catch (error) {
        const message = (error.response && error.response.data) || (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
...
export const getFollowingUsers = createAsyncThunk('user/following-user', async (data={}, thunkAPI) => {
	try {
		const { userName, page, limit, search } = data;
        return await userService.getFollowingUsers(userName, page, limit, search || "");
    }
    catch (error) {
        const message = (error.response && error.response.data) || (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
        ...
        builder
			...
			.addCase(followUser.fulfilled, (state, action) => {
				...
				if(state.publicProfile) {
					if (state.publicProfile.followers.some(flwr => flwr === state.privateProfile._id)) {
						state.publicProfile.followers = state.publicProfile?.followers.filter(flwng => flwng !== state.privateProfile._id);
					}
					else {
						state.publicProfile.followers = [state.privateProfile._id, ...state.publicProfile?.followers];
					}
				}
...
export const { resetDataState, resetUserPrivateState, resetUserPublicState, resetUserActionState, resetUserCommonState } = userSlice.actions;
