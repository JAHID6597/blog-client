...
const logout = async (token) => {
	const { data } = await axios.get(`${baseURL}/api/user/logout`, config(token));
...
const updateProfile = async (formData, token) => {
	const { data } = await axios.put(`${baseURL}/api/user`, formData, config(token));
...
const changePassword = async (formData, token) => {
	const { data } = await axios.patch(`${baseURL}/api/user/change-password`, formData, config(token));
	
    return data;
}

const followUser = async (userName, token) => {
	const { data } = await axios.post(`${baseURL}/api/user/${userName}/follow`, {}, config(token));
	
	return data;
};

const getFollowerUsers = async (userName, page, limit, search) => {
	const { data } = await axios.get(`${baseURL}/api/user/${userName}/followers?page=${page}&limit=${limit}&search=${search}`);
	
	return data;
};

const getFollowingUsers = async (userName, page, limit, search) => {
	const { data } = await axios.get(`${baseURL}/api/user/${userName}/followings?page=${page}&limit=${limit}&search=${search}`);
	
	return data;
};


const userService = { signUp, signIn, logout, getPublicProfile, getPrivateProfile, updateProfile, deleteProfile, changePassword, followUser, getFollowerUsers, getFollowingUsers };
