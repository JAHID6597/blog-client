...
export const getCategory = createAsyncThunk('adminCategory/get-category', async (slug, thunkAPI) => {
	try {
...
export const getCategories = createAsyncThunk('adminCategory/get-categories', async (data={}, thunkAPI) => {
	try {
		const token = thunkAPI.getState().admin.admin.accessToken;

		const { page, limit, search} = data;

        return await adminCategoryService.getCategories(page, limit, search || "", token);
    }
    catch (error) {
        const message = (error.response && error.response.data) || (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
...
export const createCategory = createAsyncThunk('adminCategory/create-category', async (formData, thunkAPI) => {
	try {
		const token = thunkAPI.getState().admin.admin.accessToken;
...
export const updateCategory = createAsyncThunk('adminCategory/update-category', async ({ formData, slug }, thunkAPI) => {
	try {
		const token = thunkAPI.getState().admin.admin.accessToken;

        return await adminCategoryService.updateCategory(formData, slug, token);
    }
    catch (error) {
        const message = (error.response && error.response.data) || (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
...
export const deleteCategory = createAsyncThunk('adminCategory/delete-category', async (slug, thunkAPI) => {
	try {
		const token = thunkAPI.getState().admin.admin.accessToken;
