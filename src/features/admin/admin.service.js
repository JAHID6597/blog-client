...
const logout = async (token) => {
	const { data } = await axios.get(`${baseURL}/api/admin/logout`, config(token));
...
const updateProfile = async (formData, token) => {
	const { data } = await axios.put(`${baseURL}/api/admin`, formData, config(token));
...
const adminAuthService = { signIn, logout, getPrivateProfile, updateProfile, deleteProfile }
