...
import { getLikedBlogsByPublicUser, resetDataState, resetUserLikedBlogState } from "../../features/blog/blog.slice";
...
const LikedBlogs = () => {
	...
	const { likedBlogsByPublicUser, likedBlogsMetaDataByPublicUser, resetData } = useSelector((state) => state.blog);
	const dispatch = useDispatch();
	const [page, setPage] = useState(1);

	useEffect(() => {
		if (userName) dispatch(getLikedBlogsByPublicUser({ userName, page, limit: 5, search: '' }));
