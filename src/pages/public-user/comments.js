...
import { getCommentsByPublicUser, resetUserCommentState, resetDataState } from "../../features/blog/blog.slice";
...
const Comments = () => {
	...
	const { publicUserComments, publicUserCommentsMetaData, resetData } = useSelector((state) => state.blog);
	const dispatch = useDispatch();
	const [page, setPage] = useState(1);

	useEffect(() => {
		if (userName) dispatch(getCommentsByPublicUser({ userName, page, limit: 12, search: '' }));
