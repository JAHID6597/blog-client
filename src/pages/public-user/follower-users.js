...
import { getFollowerUsers, resetUserActionState, resetDataState } from "../../features/user/user.slice";
...
const FollowerUsers = () => {
	...
	const { followers, followersMetaData, resetData } = useSelector((state) => state.user);
	const dispatch = useDispatch();
	const [page, setPage] = useState(1);

	useEffect(() => {
		if (userName) dispatch(getFollowerUsers({ userName, page, limit: 6, search: '' }));
