...
import { getBookmarkedBlogsByPublicUser, resetDataState, resetUserBookmarkedBlogState } from "../../features/blog/blog.slice";
import { resetPublicUserInfoTabItemState } from "../../features/common/common.slice";

const BookmarkedBlogs = () => {
	const { userName } = useParams();

	const { bookmarkedBlogsByPublicUser, bookmarkedBlogsMetaDataByPublicUser, resetData } = useSelector((state) => state.blog);
	const dispatch = useDispatch();
	const [page, setPage] = useState(1);

	useEffect(() => {
		if (userName) dispatch(getBookmarkedBlogsByPublicUser({ userName, page, limit: 5, search: '' }));
