...
import { resetUserActionState, resetDataState, getFollowingUsers } from "../../features/user/user.slice";


const FollowingUsers = () => {
	const { userName } = useParams();

	const { followings, followingsMetaData, resetData } = useSelector((state) => state.user);
	const dispatch = useDispatch();
	const [page, setPage] = useState(1);

	useEffect(() => {
		if (userName) dispatch(getFollowingUsers({ userName, page, limit: 6, search: '' }));
