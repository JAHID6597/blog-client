...
import { getBlog, getComments, getReadNextBlogs, resetBlogState, resetBlogCommentState, resetBlogCommonState } from "../features/blog/blog.slice";
...
const SingleBlog = () => {
	...
	const { blog, isError, isActionError, message } = useSelector((state) => state.blog);
	...
	return (
		<>
			<Container maxWidth="xl">
				<Grid container>
					<Grid item lg={1} sx={{ px: 2, display: { lg: 'block', xs: "none" } }}><LeftSidebar blog={blog} /></Grid>

					<Grid item lg={8} md={8} xs={12} sx={{ px: { md: 2, xs: 0 } }}><Main setCommentPage={setCommentPage} /></Grid>

					<Grid item lg={3} md={4} sx={{ px: { md: 2, xs: 0 }, width: '100%', mt: { md: 0, sm: 3 } }}><RightSidebar /></Grid>
				</Grid>
			</Container>
