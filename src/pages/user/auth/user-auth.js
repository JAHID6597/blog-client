...
const UserAuth = ({ isSignup }) => {	
	...
	const { isError, isUpdateSuccess, message, isLoading } = useSelector(state => state.user);
	
	const [touched, setTouched] = useState({ firstName: false, lastName: false, userName: false, email: false, password: false, confirmPassword: false });
	const [formData, setFormData] = useState({ firstName: "", lastName: "", userName: "", email: "aa@gmail.com", password: "aaAA11@@", confirmPassword: "" });
	...
	const switchMode = () => {
		setTouched({ firstName: false, lastName: false, userName: false, email: false, password: false, confirmPassword: false });
				...
				<Box sx={{ boxShadow: "rgba(99, 99, 99, 0.2) 0px 2px 8px 0px", background: '#fff' }}>
					<Container component="main" maxWidth="xs">
						<Box sx={{ py: { md: 5, xs: 3 }, display: "flex", flexDirection: "column", alignItems: "center" }} >
							<Avatar sx={{ m: 1, bgcolor: "secondary.main" }}> <Person /> </Avatar>
							<Typography component="h1" variant="h4" sx={{ textTransform: "uppercase" }}>{isSignup ? "Sign up" : "Sign in"}</Typography>
								...
								<Box sx={{ mt: { md: 5, xs: 3 } }}>
									<Grid container spacing={2}>
										...
										<Grid item xs={12}>
											...
											/>
											{errorMessage.password && <Error message={errorMessage.password} /> }
										</Grid>
										...
										{isSignup && (
											<Grid item xs={12}>
												...
												/>
												{errorMessage.confirmPassword && <Error message={errorMessage.confirmPassword} /> }
											</Grid>
										)}
			
									</Grid>
									<Button size="large" type="submit" fullWidth variant="contained" sx={{ my: 2 }}>{isSignup ? "Sign Up" : "Sign In"}</Button>
									<Box sx={{ display: "flex", justifyContent: "flex-end" }}>
										<LinkTo url={`/user/${isSignup ? 'signin' : 'signup'}`} onClick={switchMode}>
