...
import { followUser, getFollowerUsers, resetUserActionState, resetUserCommonState } from "../../../features/user/user.slice";
...
const FollowersDetails = () => {
	const { followers, followersMetaData, privateProfile, isLoading, isError, isUpdateSuccess } = useSelector((state) => state.user);
	const key = "follower";
	...
	useEffect(() => {
		dispatch(getFollowerUsers({ userName: privateProfile.userName, page, limit, search }));
	...
	return (
		<MuiDataTableLayout>
			<MUIDataTable
				title="Followers"
				data={followers}
				columns={followTableColumns(navigateToUserProfilePage, key, unFollowUser)}
				options={options(page, setPage, limit, setLimit, setSearch, isLoading, followersMetaData)}
			/>
		</MuiDataTableLayout>
	);
};
