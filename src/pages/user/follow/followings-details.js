...
import { getFollowingUsers, resetUserActionState, resetUserCommonState } from "../../../features/user/user.slice";
...
const FollowingsDetails = () => {
	const { followings, followingsMetaData, privateProfile, isLoading, isError } = useSelector((state) => state.user);
	const key = "following";
	...
	useEffect(() => {
		dispatch(getFollowingUsers({ userName: privateProfile.userName, page, limit, search }));
	...
	return (
		<MuiDataTableLayout>
			<MUIDataTable
				...
				options={options(page, setPage, limit, setLimit, setSearch, isLoading, followingsMetaData)}
			/>
		</MuiDataTableLayout>
	);
};
