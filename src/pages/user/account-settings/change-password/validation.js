const validation = (touched, formData, setSubmit) => {
    ...
    const passwordFormat = /(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@$!%*#?&^_-]).{8,}/;
    ...
    if (touched.newPassword) {
        if (!formData.newPassword) {
            ...
            error.newPassword = "Password must be at least 8 characters or longer with 1 uppercase, 1 lowercase, 1 numeric and 1 special character like @$!%*";
            isSubmit = false;
        }
    }

    if (touched.newPassword && touched.confirmPassword && formData.newPassword !== formData.confirmPassword) { 
