...
import { bookmarkBlog, getBookmarkedBlogsByPrivateUser, resetBlogCommonState, resetUserBookmarkedBlogState } from "../../../features/blog/blog.slice";
...
const BookmarkedBlogsDetails = () => {
	const { bookmarkedBlogsByPrivateUser, bookmarkedBlogsMetaDataByPrivateUser, isLoading, isError, isUpdateSuccess } = useSelector((state) => state.blog);
	...
	useEffect(() => {
		if (isUpdateSuccess) toast.success('Successfully Removed From Bookmark.');
	...
	return (
		<MuiDataTableLayout>
			<MUIDataTable
				...
				options={options(page, setPage, limit, setLimit, setSearch, isLoading, bookmarkedBlogsMetaDataByPrivateUser)}
			/>
		</MuiDataTableLayout>
	);
};
